...
superseded or revisited — what changed them and why. The index is generated by
`./scripts/adr-index.sh` and checked by `AdrIndexTest`, so a decision recorded without an
entry fails the build. The feature by feature plan is in
